      ...
      <div className="flex space-x-4 overflow-x-auto pb-4 hide-scrollbar">
        {tracks.map((track, i) => (
          <div key={i} className="flex-shrink-0 w-32">
            ...
            <h3 className="text-sm font-ui truncate text-right rtl:text-right ltr:text-left">{track.title}</h3>
            <p className="text-xs font-light-ui text-textSecondary truncate text-right rtl:text-right ltr:text-left">{track.artist}</p>
          </div>
        ))}
      </div>
    </div>
